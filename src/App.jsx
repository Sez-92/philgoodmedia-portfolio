...
const GLUTENFRY_CARDS = [
  ...
  { id: 4, label: "04 — Kampagne",       title: "Die Umsetzung",   desc: "Social Media, OOH, Print – konsistent und markenstark über alle Kanäle hinweg.",                          rot:  5,  offX:  160, topOff: 40, zi: 2 },
  { id: 5, label: "05 — Ergebnis",       title: "Das Resultat",    desc: "Ein Brand, der ankommt und beweist: glutenfrei kann großartig aussehen.",                                  rot: 11,  offX:  320, topOff: 80, zi: 1 },
];
...
const styles = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* MARQUEE */
  .marquee-section { padding: 40px 0; overflow: hidden; border-top: 1px solid rgba(255,255,255,0.06); border-bottom: 1px solid rgba(255,255,255,0.06); background: rgba(212,237,42,0.02); }
  .marquee-track { display: flex; animation: marquee 28s linear infinite; width: max-content; }
  .marquee-track:hover { animation-play-state: paused; }
  .marquee-item { font-family: 'Bebas Neue', sans-serif; font-size: 28px; letter-spacing: 4px; color: rgba(255,255,255,0.15); padding: 0 36px; white-space: nowrap; transition: color 0.3s; }
  .marquee-item:hover { color: rgba(255,255,255,0.4); }
  .marquee-item.clickable { color: ${LIME}; cursor: none; }
  .marquee-item.clickable:hover { color: ${WHITE}; }
  .marquee-badge { font-family: 'Space Mono', monospace; font-size: 8px; letter-spacing: 2px; background: ${LIME}; color: ${BLACK}; padding: 2px 7px; vertical-align: middle; text-transform: uppercase; font-weight: 700; margin-left: 8px; margin-right: 2px; transition: background 0.3s; }
    ...
    const obs = new IntersectionObserver(([e]) => {
      if (e.isIntersecting && !started.current) {
        ...
        const timer = setInterval(() => {
          s += step;
          if (s >= target) { setCount(target); clearInterval(timer); } else setCount(s);
        }, 40);
      }
    }, { threshold: 0.5 });
...
function SpreadCard({ card, hoveredCard, setHoveredCard }) {
  ...
  const cardStyle = {
    ...
    transform: isHovered
      ? "rotate(0deg) translateY(-44px) scale(1.12)"
      : `rotate(${card.rot}deg)`,
    opacity: isDimmed ? 0.45 : 1,
    filter: isDimmed ? "brightness(0.5) saturate(0.7)" : "none",
    ...
    boxShadow: isHovered
      ? `0 70px 130px rgba(0,0,0,0.98), 0 0 70px rgba(212,237,42,0.14)`
  ...
  return (
    ...
    >
      ...
      <div style={{ width: "100%", height: 210, position: "relative", overflow: "hidden", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 8, background: "rgba(255,255,255,0.025)" }}>
        ...
        <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 80, color: "rgba(212,237,42,0.09)", lineHeight: 1, position: "relative", zIndex: 1 }}>0{card.id}</div>
  ...
  return (
    <div style={{ minHeight: "100vh" }}>
        ...
        <div style={{ position: "relative", zIndex: 1 }}>
          ...
          <div style={{ display: "flex", gap: 60, marginTop: 48, paddingTop: 32, borderTop: "1px solid rgba(255,255,255,0.08)", animation: "slideUp 0.7s 0.35s both", flexWrap: "wrap" }}>
            {[
              ...
              { label: "Kategorie",value: "Brand Design · Print · Digital" },
              { label: "Rolle",     value: "Art Direction" },
              { label: "Agentur",   value: "Bounty Communication Group" },
            ].map(m => (
              <div key={m.label}>
                <label style={{ fontFamily: "'Space Mono', monospace", fontSize: 9, letterSpacing: 2, textTransform: "uppercase", color: GRAY, display: "block", marginBottom: 6 }}>{m.label}</label>
                <span style={{ fontSize: 15, color: WHITE, fontWeight: 500 }}>{m.value}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{ padding: "100px 40px 80px" }}>
        ...
        <div style={{ position: "relative", height: 520, width: "100%", overflowX: "visible" }}>
          {GLUTENFRY_CARDS.map(card => (
            <SpreadCard key={card.id} card={card} hoveredCard={hoveredCard} setHoveredCard={setHoveredCard} />
          ))}
        </div>

        <p style={{ fontFamily: "'Space Mono', monospace", fontSize: 9, letterSpacing: 3, textTransform: "uppercase", color: "rgba(255,255,255,0.18)", marginTop: 88, textAlign: "center" }}>
          ← Hover über die Karten →
        </p>
      </section>
...
/* Liest die aktuell im Browser-Verlauf/Hash hinterlegte Seite aus */
function getPageFromLocation() {
  return window.location.hash === "#glutenfry" ? "glutenfry" : "home";
}
...
export default function Portfolio() {
  ...
  /* ── Seitenwechsel (Animation) OHNE History-Eintrag ── */
  const applyPageChange = (page) => {
  ...
  /* ── Seitenwechsel MIT History-Eintrag (für Klicks in der App) ── */
  const navigateTo = (page) => {
    applyPageChange(page);
    const url = page === "glutenfry" ? "#glutenfry" : (window.location.pathname + window.location.search);
    window.history.pushState({ page }, "", url);
  };

  const scrollTo = (id) => document.getElementById(id)?.scrollIntoView({ behavior: "smooth" });

  /* ── Browser Zurück/Vor-Button abfangen ── */
  useEffect(() => {
    // sorgt dafür, dass der allererste History-Eintrag ebenfalls die aktuelle Seite kennt
    window.history.replaceState({ page: currentPageRef.current }, "", window.location.hash === "#glutenfry" ? "#glutenfry" : (window.location.pathname + window.location.search));

    const onPopState = (e) => {
      const target = (e.state && e.state.page) || getPageFromLocation();
      if (target !== currentPageRef.current) {
        applyPageChange(target);
      }
    };
  ...
  return (
    <div className="portfolio-root">
      ...
      <nav className={`nav${scrolled ? " scrolled" : ""}`}>
        <div className="nav-logo" onClick={() => currentPage === "glutenfry" ? navigateTo("home") : scrollTo("hero")}>PS</div>
        {currentPage === "glutenfry" ? (
          <button className="nav-back" onClick={() => navigateTo("home")}>
            <span className="nav-back-arrow">←</span> Alle Projekte
          </button>
        ) : (
            ...
            <div className="marquee-section">
              <div className="marquee-track">
                {[...marqueeItems, ...marqueeItems].map((item, i) =>
                  item === "Glutenfry" ? (
                    <span key={i} className="marquee-item clickable" onClick={() => navigateTo("glutenfry")}>
                ...
                <FadeIn>
                  <div className="contact-wrapper">
                    ...
                    <p style={{ fontSize: 15, color: GRAY, maxWidth: 440, margin: "0 auto", lineHeight: 1.8, fontWeight: 300 }}>
                      Ich bin kein Schema-F-Mensch – und suche Projekte, die das auch nicht sind. Schreib mir, ruf an, oder schau dir mein Portfolio an.
                    </p>
